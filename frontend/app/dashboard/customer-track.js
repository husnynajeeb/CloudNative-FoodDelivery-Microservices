...
const SOCKET_URL = 'http://192.168.106.55:5003'; // Your delivery socket server
...
const CustomerTrackScreen = () => {
  ...
  useEffect(() => {
    const fetchOrderTracking = async () => {
      try {
        const response = await fetch(`http://192.168.106.55:5001/api/orders/${orderId}/tracking`);
  ...
  const fetchRoute = async (origin, destination) => {
    try {
      ...
      if (data.routes.length > 0) {
        const points = polyline.decode(data.routes[0].overview_polyline.points).map(([latitude, longitude]) => ({ latitude, longitude }));
      ...
      <View style={styles.infoPanel}>
        <View style={styles.infoPanelHeader}>
          <Text style={styles.infoPanelTitle}>
            {orderDelivered ? 'Order Delivered' : 'Order Tracking'}
          </Text>
        </View>
          ...
          <View style={styles.infoBox}>
            ...
            <View>
              <Text style={styles.infoLabel}>
                {orderDelivered ? 'Delivered At' : 'ETA'}
              </Text>
