...
export default function AddMenuForm() {
  ...
  const { user, token } = useAuthStore(); // Access both user and token directly

  useEffect(() => {
    // Check authentication status on component mount
    const checkAuth = async () => {
      const authToken = token || user?.token || user?.authToken; // Try multiple possibilities
      if (!authToken) {
        setMessage('Authentication token not found. Redirecting to login...');
        setTimeout(() => {
          router.replace('/Screen/Auth/Login'); // Redirect to login screen
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Navigate to ViewMenuForm with the new menu item's ID
      setTimeout(() => {
        const newItemId = response.data.data._id;
        router.push(`/Screen/Menu/ViewMenuForm?id=${newItemId}`);
      }, 1000);
    } catch (err) {
