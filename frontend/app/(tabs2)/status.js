...
export default function RestaurantDashboard() {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      ...
      await axios.patch(
        `http://${BASE_IP}:5002/api/restaurant-menu/orders/${orderId}/status`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        {["pending", "accepted", "preparing"].includes(item.status) && (
          ...
          >
            {updatingOrderId === item._id ? (
              ...
              <>
                <Feather name="truck" size={18} color="#FFFFFF" style={{ marginRight: 8 }} />
                <Text style={styles.updateButtonText}>Mark as Dispatched</Text>
              </>
            )}
          </TouchableOpacity>
        )}
        ...
        <View style={styles.contentContainer}>
          <View style={styles.titleContainer}>
            <View style={styles.iconContainer}>
              <MaterialIcons name="delivery-dining" size={24} color="#3563E9" />
            </View>
